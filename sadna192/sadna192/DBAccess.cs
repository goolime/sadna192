...
/**************** Update DB ****************/ 
        ...
        internal static bool updateManager(Member m, Store s, bool permision_add, bool permission_remove, bool permission_update)
        {
            bool ans = false;
            try
            {
                using (var ctx = new Model1())
                {
                    Manager res = (Manager)ctx.Owners.Include("Store").Include("User")
                                    .Where(p => p.user.name == m.name
                                    && p.store.name == s.name)
                                    .FirstOrDefault();
                    if (res == null)
                        throw new Sadna192Exception("manager is not exist for this store", "DBAccess", "updateManager");
                    res.permision_add = permision_add;
                    res.permision_remove = permission_remove;
                    res.permision_update = permission_update;
                    ctx.SaveChanges();
                }
            }
            catch (DbUpdateException e)
            {
                Console.WriteLine("manager is not exist for this store" + e.ToString());
            }
            return ans;
        }
